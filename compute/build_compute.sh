...
DOCKERFILE=${DOCKERFILE:-"./build/compute.Dockerfile"}

echo "Building $IMAGE_NAME from Dockerfile=$DOCKERFILE at $(date) ... "
docker build -t ${IMAGE_NAME} -f ${DOCKERFILE} \
    --build-arg http_proxy=$http_proxy --build-arg https_proxy=$https_proxy ./build

if [ $? = 0 ] ; then
    docker images $IMAGE_NAME
    echo "Docker image $IMAGE_NAME built successfully."
    echo "To quickly test it, you can launch it with:"
    echo "docker run -it --rm --env http_proxy=$http_proxy --env https_proxy=$https_proxy --env no_proxy=$no_proxy $IMAGE_NAME /bin/bash"
else
